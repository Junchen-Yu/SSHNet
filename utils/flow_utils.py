...
def disp_to_flow(four_point, coords, downsample=4):
    ...
    flow = torch.cat((points_new[:, 0, :].reshape(coords.shape[0], coords.shape[3], coords.shape[2]).unsqueeze(1),
...
class Get_Flow(nn.Module):
    ...
    def forward(self, four_point, a):
        four_point = four_point/ torch.Tensor([a]).to(four_point.device)
